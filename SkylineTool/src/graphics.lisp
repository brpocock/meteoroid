...
(defun colu-from-color (color)
  (let ((co (/ (logand #xf0 color) #x10))
        (lu (logand #x0e color)))
    (format nil "COL~a, $~x"
            (elt '(grey yellow brown orange red magenta purple indigo
...
(defun bit-vector-to-byte (bit-vector)
  (reduce (lambda (byte bit) (logior (ash byte 1) bit))
          bit-vector))

(defun pixels-to-byte (pixels x y0 y1)
  (bit-vector-to-byte 
   (coerce (loop for y from y0 upto y1
...
(defun map-spans-to-bytes (pixels)
  (flatten (loop for x from 0 below (array-dimension pixels 0)
                 collecting (vertical-span-to-bytes pixels x))))
...
(defun find-splits-in-map-row (pixels)
  (loop for column from 0 below (array-dimension pixels 0)
        if (column-is-solid-p pixels column)
          collect column))

(defun find-horizontal-spans (pixels)
  (let ((splits (find-splits-in-map-row pixels)))
    (let ((span-starts (list))
          (span-ends (list)))
      (loop for i from 0 below (1- (length splits))
            for split = (elt splits i)
            for next-split = (elt splits (1+ i))
            do (progn (push split span-starts)
                      (push next-split span-ends)))
      (loop for i from (1- (length span-starts)) downto 0
